...
// Fallback icons per category for image placeholder
const categoryIcons: Record<string, string> = {
  cpu: 'Cpu', gpu: 'Monitor', ram: 'MemoryStick', storage: 'HardDrive',
  motherboard: 'CircuitBoard', psu: 'Zap', case: 'Box', cooler: 'Wind',
};

function ComponentImage({ src, alt, categoryId }: { src: string; alt: string; categoryId: string }) {
              ...
              >
                ...
                {i === activeBuildIdx && (
                  <div className="flex items-center pr-2 gap-1">
                    <button className="p-1 opacity-60 hover:opacity-100" onClick={() => setEditingName(b.id)}>
                      <Icon name="Pencil" size={11} />
                    </button>
                    {builds.length > 1 && (
                      <button className="p-1 opacity-60 hover:opacity-100" onClick={() => removeBuild(i)}>
                        <Icon name="X" size={11} />
                      </button>
                    )}
                  </div>
